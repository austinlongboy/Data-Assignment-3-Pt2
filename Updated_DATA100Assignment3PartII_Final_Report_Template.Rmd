...
final_join <- covid_happy |>
  left_join(ice_clean |> select(hemisphere, Extent), by = "hemisphere")

ggplot(final_join, aes(x = Extent, y = score)) +
  geom_point(alpha = 0.6) +
  labs(
    title = "Happiness Score vs Sea Ice Extent",
    x = "Ice Extent (North/South Hemisphere)",
    y = "Happiness Score"
  ) +
  theme_minimal()
